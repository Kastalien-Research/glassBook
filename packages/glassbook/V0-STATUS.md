...
- 🟡 **initialize** (`src/sections/initialize.mts`): single structured call with **no tools**, so gates are *guessed* from conventions unless pinned with `--gate`. Unpinned gate quality is the biggest reliability risk.
