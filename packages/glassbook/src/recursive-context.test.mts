...
const config: RunConfig = {
  prompt: 'answer from prior notebook',
  repoDir: '/tmp/repo',
...
describe('askContext', () => {
  ...
  it('rejects uncited or invalid recursive answers', async () => {
  ...
  it('records recursive calls into glassBook state and typed cells', async () => {
    const state = initialState(config);
    const result = await askContext({
      parentCellId: 'research-cell',
      question: 'What artifact?',
      refs: [{ ref, content }],
      selectors: [{ query: 'Needle', maxSpans: 1 }],
      responder: async (request) => ({
        answer: 'earnings-update recipe notebook',
        citations: [
          {
            refId: request.spans[0].refId,
            sourcePath: request.spans[0].sourcePath,
    ...
    recordRecursiveContextCall(state, result.value.call);

    expect(state.recursiveContextCalls).toEqual([result.value.call]);
    expect(state.glassbookCells).toHaveLength(1);
    expect(state.glassbookCells[0]).toMatchObject({
      section: 'research',
      input: { parentCellId: 'research-cell', question: 'What artifact?' },
      processing: { depth: 1, refs: [ref.id] },
      output: { status: 'ok', answer: 'earnings-update recipe notebook' },
      gates: [],
    });
  });
});
